...
function renderPanelTitle(panelId, switchId, btnId, titleText, links = [], stateKey, btnTexts) {
    const panel = document.getElementById(panelId);
    if (!panel) return;

    let title = panel.querySelector('.rebate-title');
    // 不存在 → 创建
    if (!title) {
    ...
    // 更新标题
    const titleSpan = title.querySelector('.panel-title-text');
    const switchContainer = title.querySelector(`#${switchId}`);
    const switchBtn = title.querySelector(`#${btnId}`);
